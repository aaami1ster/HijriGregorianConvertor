...
/**
 * contain Gregorian calendar functions and constants
 * @author abdalla
 *
 */
public class GreCalendar {
	...
	/**
	 * given Hijri date get the equivalent Gregorian date.
	 * @param y year
	 * @param m month
	 * @param d day
	...
	private static void GetGregorain(long absDays) {
		...
		int monthGre = 1;
		//long daysOfGreMonth = GetDaysInGreMonth(yearGre,monthGre);
		//use for loop instead of while to solve bug of 1/1/ result in two days
		//0day and 1day
		// solve 13 month bug by removing = in gremonth<12
		//remove daysOfGreMonth and use GetDaysInGreMonth directly to solve the bug of 19/7/2012 -> 30/8/1433 but 20/7/2012 -> 2/9/1433
		for(monthGre=1;monthGre < 12 && LeftGreDays>GetDaysInGreMonth(yearGre,monthGre);monthGre++){
			//daysOfGreMonth = GetDaysInGreMonth(yearGre,monthGre);
	...
	public static int CheckYearMonthDayRangeG(int year, int month, int day) throws Exception{
		//CheckYearRangeG(year);
		if(year < MIN_GRE_YEAR || year > MAX_GRE_YEAR){
			//throw new Exception("GreYear out of range");
			return -1;
		}else if(year == MAX_GRE_YEAR){
			if(month > MAX_GRE_MONTH){
				//throw new Exception("GreMonth out of range");
				return -2;
		...
		if(month < 1 || month > 12){
			//throw new Exception("GreDay out of range");
			return -6;
		}
