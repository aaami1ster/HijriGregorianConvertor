...
public class HijriCalendar {
	...
	private static void GetHijri(long absDays){
		...
		//////////////////calculate month
		int monthHijri = 1;
		//long daysOfHijriMonth = GetDaysInHijriMonth(yearHijri,monthHijri);
		//when <= result in 13 month
		for(monthHijri=1;monthHijri < 12 && LeftHijriDays>GetDaysInHijriMonth(yearHijri,monthHijri);monthHijri++){
			//daysOfHijriMonth = GetDaysInHijriMonth(yearHijri,monthHijri);
			//System.out.println(LeftHijriDays +":"+daysOfHijriMonth+":"+monthHijri);
			LeftHijriDays -= GetDaysInHijriMonth(yearHijri,monthHijri);
		}
		/*
		while(LeftHijriDays>daysOfHijriMonth && monthHijri<12){
			LeftHijriDays -= daysOfHijriMonth;
			monthHijri++;
			daysOfHijriMonth = GetDaysInHijriMonth(yearHijri,monthHijri);
		}
		*/
